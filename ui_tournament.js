...
(function(){
  ...
  async function handleShowTeamList(payload){
    ...
    const teams = Array.isArray(payload?.teams) ? payload.teams : (st.teams || []).map(t=>({ id:t.id, name:t.name, power:t.power, isPlayer:t.isPlayer }));
    ...
    // start auto scroll (loop-ish)
    busy = true;
    setNextEnabled(false);

    // compute scroll duration based on content height
  ...
  async function handleShowCoachSelect(payload){
    ...
    setNextEnabled(false); // 選択で進む（NEXTは使わない）
  }
  ...
  async function handleShowEvent(payload){
    ...
    // 表示テンポ：3段固定は flow が center3 で渡している
    const st = getState();
    if (st?.ui?.center3){
      setLines(st.ui.center3[0], st.ui.center3[1], st.ui.center3[2]);
    }else{
  ...
  async function handlePrepareBattles(){
    // ここはUIは何もしない（次の step で showEncounter / cpuBattleSilent が来る）
    setNextEnabled(true);
  }
  ...
  async function handleShowEncounter(payload){
    ...
    // エリア背景は今いるbgのまま（交戦中レイアウト）
    // 中央上に brbattle を出すのは battle 本番で。接敵はスプラッシュで派手に。
    setBanners(st.bannerLeft, st.bannerRight);
    ...
    // right team image naming rule: teamId.png（フォルダは既存運用に合わせる）
  ...
  function pickChats(n){
    ...
    // ウルト台詞（仕様：ウルト使う時は必ず）…現状は確定のタイミング情報が無いので、10個中1回だけ混ぜる
    // ※後で「ウルト発動フラグ」が来たらそこに合わせる
    if (out.length >= 6){
  ...
  async function handlePlayerEliminatedFastForward(){
    // UI側は一瞬だけ表示して、result request が来るまで step を回す
    setNextEnabled(false);
    busy = true;
    ...
    // もう flow 側で fastForward されて phase が match_result になる想定。
    // 念のため step を1回回して request を更新
    flow.step();
    busy = false;
    render();
  }
  ...
  async function handleShowArrive(payload){
    // showArriveは showMove の後に来る場合もあるが、来たらそれに従う
    hidePanels();
    hideSplash();
    showCenterStamp('');
    setEventIcon('');
  ...
  async function handleShowMatchResult(payload){
    ...
    // result background: battle.png (square) を仕様に合わせる
    // ※ battle.png が存在する前提（コンセプトにあった）
  ...
  async function handleShowTournamentResult(payload){
    ...
    // tournament result: simple sort by sumTotal desc
  ...
  async function render(){
    ...
    // base visuals from state
    setBanners(st.bannerLeft, st.bannerRight);

    // If flow stores ui.bg etc
    ...
    if (!req || !req.type){
      // no request: allow next
      setNextEnabled(!busy);
      return;
    }

    // request-driven handlers
    try{
      switch(req.type){
  ...
  function initTournamentUI(){
    ensureDom();
    // 外側から open/render を呼ばれる想定だが、念のため
  }
