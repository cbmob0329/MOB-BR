/* =========================================================
   ...
   - “大会終了後処理” は tournamentCorePost があれば呼ぶ（LOCALは結果表示の次NEXTで実行）
   ========================================================= */
'use strict';
...
(function(){
  ...
  function _callNationalPostHook(){
    if (window.MOBBR?.sim?.tournamentCorePost?.onNationalTournamentFinished){
  ...
  function step(){
    ...
    // ✅ 大会結果画面の「次のNEXT」で終了後処理を実行して閉じる
    if (state.phase === 'tournamentResult_local_post'){
      _callLocalPostHook();
      setRequest('endTournament', {});
      state.phase = 'done';
      return;
    }
    if (state.phase === 'tournamentResult_national_post'){
      _callNationalPostHook();
      setRequest('endTournament', {});
      state.phase = 'done';
      return;
    }
    ...
    if (state.phase === 'match_result_done'){
      ...
      if (state.mode === 'local'){
        if (state.matchIndex >= state.matchCount){
          ...
          // ✅ 次のNEXTで post hook → endTournament
          state.phase = 'tournamentResult_local_post';
          return;
        }
      ...
      if (state.mode === 'national'){
        ...
        if (isLastSession){
          // ✅ まず総合結果を表示
          setRequest('showTournamentResult', { total: state.tournamentTotal });
          // ✅ 次のNEXTで post hook → endTournament（現状のpostは未使用でもOK）
          state.phase = 'tournamentResult_national_post';
          return;
        }
