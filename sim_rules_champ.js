/* =========================================================
   sim_rules_champ.js (FULL)
   - チャンピオンシップ（Championship League）ルール定義
   - 依存：
...
(() => {
  ...
  if (!CORE) throw new Error('SIM_TOURNAMENT_CORE not found. Load sim_tournament_core.js before sim_rules_champ.js');
  ...
  // ---------------------------------------------
  // 参加チーム構成（Championship）
  //
  // 11月第1週  ローカル大会（20/5試合） → 上位10がナショナルへ
  // 12月第1週  ナショナル大会（40/複数試合）→ 上位8 WF確定 / 9〜28 LC / 29〜40 敗退
  // 12月第3週  ラストチャンス（9〜28の20/5試合）→ 上位2 WFへ
  // 1月第2週   チャンピオンシップWF（40/複数ステージ）→ Final
  // ---------------------------------------------
  ...
  // 実行（Championship）
  // ---------------------------------------------
  function runCH_LocalTournament(state) {
  ...
  function runCH_NationalTournament(state, localTop10Names) {
  ...
  function runCH_LastChance(state, nationalRanking40) {
  ...
  function runCH_WorldFinal(state, nationalRep10Names) {
    const teams40 = buildWorldFinal40Teams(state, nationalRep10Names);

    // ワールドファイナル：ステージ式（SP1/SP2と同様の簡易版で一旦完成）
  ...
  const api = {
    runLocal: () => {
      const st = getState();
      const res = runCH_LocalTournament(st);
      saveResultToState('CH_LOCAL', res);
      return res;
    },

    runNational: (localTop10Names) => {
      const st = getState();
      const res = runCH_NationalTournament(st, localTop10Names);
      saveResultToState('CH_NATIONAL', res);
      return res;
    },

    runLastChance: (nationalRanking40) => {
      const st = getState();
      const res = runCH_LastChance(st, nationalRanking40);
      saveResultToState('CH_LAST_CHANCE', res);
      return res;
    },

    runWorldFinal: (nationalRep10Names) => {
      const st = getState();
      const res = runCH_WorldFinal(st, nationalRep10Names);
      saveResultToState('CH_WORLD_FINAL', res);
      return res;
    },

    _internal: {
      runCH_LocalTournament,
      runCH_NationalTournament,
      runCH_LastChance,
      runCH_WorldFinal,
    }
  };
