/* =========================================================
   MOB BR - sim_tournament_final.js (FULL / FIXED)
   ---------------------------------------------------------
   役割：
   ・ファイナル（既定：20チーム / 5試合）※20/40両対応
...
(function(){
  ...
  const Final = {};
  window.MOBBR.sim.tournamentFinal = Final;

  // ---------------------------------------------------------
  // CONFIG / KEYS
  // ---------------------------------------------------------
  const LS_KEY = 'mobbr_tournament_final_state_v1';
   ...
   *  - teamCount (number) : 20 or 40（既定20）
   *  - matchTotal (number) : 既定5
   *  - teams (array) : 外部注入（任意）
   *  - seed (number) : 任意
   */
  Final.create = function(opt){
    const o = opt || {};
    if (o.keepStorage !== false){
      const saved = readState();
      if (saved && saved.kind === 'final' && saved.matchIndex < saved.matchTotal){
        return saved;
      }
    }

    const teamCount = (Number(o.teamCount) === 40) ? 40 : 20;
   ...
   */
  Final.playNextMatch = function(state, opt){
    const st = state || readState();
    if (!st || st.kind !== 'final') return null;
    ...
    if (o.openUI !== false){
      ...
      if (ui && typeof ui.open === 'function'){
        ui.open({
          ...
          subtitle: String(o.subtitle || `ファイナル 第${matchNo}試合`),
          matchIndex: matchNo,
   ...
   * 総合順位UI
   */
  Final.openOverallUI = function(state, opt){
    const st = state || readState();
    if (!st || st.kind !== 'final') return;
    ...
    ui.open({
      ...
      subtitle: String(o.subtitle || `ファイナル 現在順位（${st.matchIndex}/${st.matchTotal}）`),
  ...
  Final.isFinished = function(state){
    const st = state || readState();
    return !!(st && st.kind === 'final' && st.matchIndex >= st.matchTotal);
  };

  Final.getFinalOverall = function(state){
    const st = state || readState();
    if (!st || st.kind !== 'final') return null;
  ...
  function buildDefaultTeams(teamCount){
    ...
    const cpuPool = getFinalCpuTeams();
  ...
  function getFinalCpuTeams(){
    ...
    // “final” っぽいのを優先（命名揺れ対応）
    const hit = (t)=>{
      const id = String(t.teamId||'').toLowerCase();
      if (!id) return false;
      return id.startsWith('final') || id.includes('final') || id.startsWith('f_') || id.startsWith('fn');
    };
